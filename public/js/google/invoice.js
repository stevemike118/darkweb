...
auth.onAuthStateChanged(user => {
	...
	if (user.displayName && user.email) {
		...
		emailP.innerHTML = `
			An email invoice will be sent to: <span>${user.email}</span>
		`;
		...
		emailP.innerHTML = `
			A dynamic link will be sent to: <span>${user.phoneNumber}</span>
		`;
	}  else if(user.phoneNumber && !user.displayName) {
...
const depositBtc = (even) => {
	...
	if(inputAmount.value >= 10 && inputAmount.value <= 500) {
		...
		alert('Enter an amount between $10 and $500');
	}
}
